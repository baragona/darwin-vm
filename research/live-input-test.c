...
#undef main
#include <assert.h>
static int sent,fail,fail_at,allocations,minimum_allocations;
int live_display_frame(int full){(void)full;return 1;}
void live_display_close(void){}
uint64_t virtual_touch_start(void){return 1;}
int virtual_touch_dispatch(void *e){(void)e;assert(allocations>=minimum_allocations);sent++;return !fail&&sent!=fail_at;}
...
static void *f(void*a,uint64_t b,unsigned c,unsigned d,unsigned e,double i,double j,double k,double l,double m,unsigned n,unsigned o,unsigned p){allocations++;return h(a,b,c,d,e,0,0,i,j,k,l,m,n,o,p);}
...
int main(void){
 hand=h;finger=f;keyboard=k;now=tick;release=rel;integer=integer_stub;sender=sender_stub;timestamp=sender_stub;append=append_stub;dispatch=dispatch_stub;
 ...
 minimum_allocations=allocated+2;
 assert(command("T .2 .3"));assert(sent==before+2&&!touching);
 assert(allocations==allocated+2);minimum_allocations=0;
 fail_at=sent+2;assert(!command("T .4 .5"));assert(touching&&last_x==.4);
