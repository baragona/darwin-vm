...
def prepare():
 ...
 p,f=current();assert f.FindRegister('pc').GetValueAsUnsigned()==0x24851feb0
...
def call(op):
 ...
 reg(f,'pc',0x24851aeb8)
 print('PERSONA_SYSCALL_READY',op)
def report(label):
 p,f=current();assert f.FindRegister('pc').GetValueAsUnsigned()==0x24851aebc
 ...
 results.append(r)
 with open('/tmp/a19-persona-syscall-v79.json','w') as out:json.dump(results,out,indent=2)
