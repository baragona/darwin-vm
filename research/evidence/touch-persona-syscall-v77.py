# V77-only controlled syscall experiment, invoked while stopped at Touch Probe spawn.
...
def prepare():
 ...
 p,f=current();assert f.FindRegister('pc').GetValueAsUnsigned()==0x23a823eb0
...
def call(op):
 ...
 reg(f,'pc',0x23a81eeb8)
 print('PERSONA_SYSCALL_READY',op)
def report(label):
 p,f=current();assert f.FindRegister('pc').GetValueAsUnsigned()==0x23a81eebc
 ...
 results.append(r)
 with open('/tmp/a19-persona-syscall-v77.json','w') as out:json.dump(results,out,indent=2)
