...
def call(op):
 ...
 reg(f,'pc',0x23a58aeb8)
 print('PERSONA_SYSCALL_READY',op)
def report(label):
 p,f=current();assert f.FindRegister('pc').GetValueAsUnsigned()==0x23a58aebc
 ...
 results.append(r)
 with open('/tmp/a19-persona-syscall-v81.json','w') as out:json.dump(results,out,indent=2)
...
def touch_spawn(frame,bp_loc,internal_dict):
 ...
 if e.Success() and name=='/Applications/Notebook.app/Notebook':
  print('TOUCH_SPAWN_READY');return True
 return False

def spawn(frame,bp_loc,internal_dict):
 p=frame.GetThread().GetProcess();e=lldb.SBError()
 name=p.ReadCStringFromMemory(frame.FindRegister("x1").GetValueAsUnsigned()&0x00ffffffffffffff,512,e)
 if e.Success() and name=="/Applications/Notebook.app/Notebook":
  print("NOTEBOOK_SPAWN",name);return True
 return False
