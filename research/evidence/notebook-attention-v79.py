...
def skip(frame,bp_loc,internal_dict):
 ...
 row={'count':count,'pc':hex(entry),'method':{0x2351ed4dc:'SBBacklightIdleTimer _reconfigureAttentionClientAndReset:',0x2351ebfe0:'SBIdleTimerGlobalCoordinator _setIdleTimerWithDescriptor:forReason:'}.get(entry,'unknown'),'lr':hex(lr),'write_success':ok}
 with open('/tmp/a19-attention-v79.jsonl','a') as f:f.write(json.dumps(row)+'\n')
