...
def skip(frame,bp_loc,internal_dict):
 ...
 row={'count':count,'pc':hex(entry),'method':{0x2398614dc:'SBBacklightIdleTimer _reconfigureAttentionClientAndReset:',0x23985ffe0:'SBIdleTimerGlobalCoordinator _setIdleTimerWithDescriptor:forReason:'}.get(entry,'unknown'),'lr':hex(lr),'write_success':ok}
 with open('/tmp/a19-attention-v78.jsonl','a') as f:f.write(json.dumps(row)+'\n')
