...
def persona(frame,bp_loc,internal_dict):
 ...
 try:
  if mobile is None:
   ...
   Path('/tmp/a19-persona-resolved-v83.json').write_text(json.dumps(row,indent=2)+'\n');print('PERSONA_RESOLVED',row)
...
def launchd(frame,bp_loc,internal_dict):
 ...
 bp=t.GetBreakpointAtIndex(t.GetNumBreakpoints()-1);bp.SetScriptCallbackFunction('a19_extension_stat_v83.fix')
 row={'launchd_base':hex(candidate),'stat_hook':hex(candidate+0x29d0c),'breakpoint':bp.GetID()}
 Path('/tmp/a19-launchd-resolved-v83.json').write_text(json.dumps(row,indent=2)+'\n');print('LAUNCHD_RESOLVED',row)
