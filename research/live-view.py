...
class Frames:
    ...
    def line(self, line):
        if line.startswith((b'LIVE_FRAME_BEGIN ', b'LIVE_PATCH_BEGIN ')):
            self.current = None
            try:
                fields = line.split()
                patch = fields[0] == b'LIVE_PATCH_BEGIN'
                if patch:
                    _, seq, parent, cw, ch, x, y, w, h, raw, packed, crc = fields
                    parent, cw, ch, x, y = map(int, (parent, cw, ch, x, y))
                else:
                    _, seq, w, h, raw, packed, crc = fields
                    parent, cw, ch, x, y = None, int(w), int(h), 0, 0
                seq, w, h, raw, packed = map(int, (seq, w, h, raw, packed))
                if not (seq > 0 and 0 < cw <= 4096 and 0 < ch <= 4096 and cw*ch*4 <= MAX_RAW
                        and 0 <= x < cw and 0 <= y < ch and 0 < w <= cw-x and 0 < h <= ch-y
                        and raw == w*h*4 and 0 < packed <= MAX_RAW+65536):
                    raise ValueError('frame bounds')
                if patch and (self.needs_keyframe or parent != self.sequence or seq <= parent
                              or self.dimensions != (cw, ch) or self.canvas is None):
                    raise ValueError('missing patch base')
                self.current = dict(seq=seq, w=w, h=h, raw=raw, packed=packed, crc=int(crc,16),
                                    data=bytearray(), patch=patch, cw=cw, ch=ch, x=x, y=y)
            except ValueError:
                self.reject()
            ...
            try:
                ...
                if int(seq) != c['seq'] or int(offset) != len(c['data']) or not data or len(data)>768 or len(c['data'])+len(data)>c['packed']:
                    raise ValueError('frame sequence')
                c['data'].extend(data)
            except (ValueError, binascii.Error):
                self.reject()
        elif line.startswith(b'LIVE_FRAME_END '):
            c = self.current
            self.current = None
            try:
                _, seq = line.split()
                if c is None or int(seq) != c['seq'] or len(c['data']) != c['packed'] or zlib.crc32(c['data']) != c['crc']:
                    raise ValueError('frame checksum')
                d = zlib.decompressobj()
                raw = d.decompress(c['data'], c['raw']+1)
                if len(raw) != c['raw'] or not d.eof or d.unused_data or d.unconsumed_tail:
                    raise ValueError('frame size')
                if c['patch']:
                    canvas = bytearray(self.canvas)
                    for row in range(c['h']):
                        dest = ((c['y']+row)*c['cw']+c['x'])*4
                        source = row*c['w']*4
                        canvas[dest:dest+c['w']*4] = raw[source:source+c['w']*4]
                    raw = bytes(canvas)
                picture = png(raw, c['cw'], c['ch'])
                self.canvas, self.dimensions = raw, (c['cw'], c['ch'])
                self.sequence, self.picture = c['seq'], picture
                self.needs_keyframe = False
                return self.sequence, picture
            except (ValueError, zlib.error):
                self.reject()
        elif line.startswith(b'LIVE_FRAME_SAME '):
            try:
                _, seq = line.split()
                if self.current is not None or self.needs_keyframe or int(seq) != self.sequence or self.picture is None:
                    raise ValueError('missing unchanged frame base')
                return self.sequence, self.picture
            except ValueError:
                self.reject()
        return None
...
class Bridge:
    def __init__(self, path, transcript, start, rate):
        ...
        self.version=1;self.parser=Frames();self.picture=None;self.frame=0;self.frame_at=0;self.status='Waiting for guest agent';self.stopped=False
    ...
    def writer(self):
        ...
        while not self.stopped:
            ...
            if cmd is None and not touch_active and not held_keys and time.monotonic()-last_frame>=1:
                cmd='G' if self.version>=2 and self.parser.needs_keyframe else 'F';last_frame=time.monotonic()
            ...
            if cmd not in ('F','G'):last_input=time.monotonic()
            self.ack.clear()
            try:self.send(cmd+'\n')
